...
def roll_back(current_angles, changes, i, new_angle, limits):
    if (
        i == 1
        and new_angle < limits[0]
        and (current_angles[0] + STEP > SERVOS_LIMITS[0][1])
    ):
        return 1
    elif (
        i == 1
        and new_angle > limits[1]
        and (current_angles[0] - STEP < SERVOS_LIMITS[0][0])
    ):
        return 1
    return 0
...
def update_angles(current_angles, changes):
    ...
    for i in range(SERVOS_NUMBER):
        ...
        if roll_back(current_angles, changes, i, new_angle, limits):
...
def add_one_object(img, box, obj, obj_name, colors_map):
    ...
    cv2.circle(
        img, (int(obj_center_x), int(obj_center_y)), 10, (0, 0, 255), 2
    )

    cv2.putText(
        img,
        f"{obj_name} Dis X: {distance_x} Dis Y: {distance_y}",
        (int(x1), int(y1) - 10),
...
def find_executable_path(base_path, system):
    ...
    executable_folder = os.path.join(base_path, "server_part", system, arch_folder)
...
def main():
    ...
    while True:
        ...
        colors_map = {
            key: generate_color(key) for key, _ in enumerate(CLASSES)
        }
        ...
        if is_command:
            if detections.shape[2] != 0:
                img_cpy = img.copy()
                img_cpy, available_classes = draw_detections(
                    img_cpy, detections, CLASSES, colors_map=colors_map
                )
