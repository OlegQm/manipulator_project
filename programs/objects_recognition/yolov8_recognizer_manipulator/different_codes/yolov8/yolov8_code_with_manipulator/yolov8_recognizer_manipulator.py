...
MODEL_PATH = "models/yolov8_model_v3.pt"
...
def main():
    ...
    while True:
        ...
        results = model.track(img, iou=iou, conf=thres, imgsz=416,
                              verbose=False, tracker="botsort.yaml", classes=name_index)
        is_command = check_for_img_command(COMMAND_FILE_PATH)
        available_classes = set()
        
        if is_command:
            results_to_send = model.track(img, iou=iou, conf=thres, imgsz=416,
        ...
        if results[0].boxes.id is not None:
            ...
            add_one_object(img, track_box, track_class,
                           class_names[track_class], colors_map)
